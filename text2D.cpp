...
void Text2D::RenderText(void)
{
	...
	for(unsigned int i=0; i < s2d.size(); i++)
	{
		...
		for(unsigned int j = 0;j < s2d[i].s.length(); j++)
		{
			...
			vertices[current_char * 6 + 2].Color = tempc;
			vertices[current_char * 6 + 3].Color = tempc;
			vertices[current_char * 6 + 4].Color = tempc;
			vertices[current_char * 6 + 5].Color = tempc;
